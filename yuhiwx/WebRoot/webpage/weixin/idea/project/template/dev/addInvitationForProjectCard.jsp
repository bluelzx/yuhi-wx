	    		...
	    		<%--
				   <li  class="mui-table-view-cell mui-media">
				            <div class="mui-media-body">
				                <span>你好<span> 
				                <button style="float: right" class="mui-btn-danger">删除</button>
				                <p class='mui-ellipsis' onclick="删除(this)">收费标注：20 &nbsp;&nbsp;&nbsp;<strong>现收费：</strong>100.0元 </p>
				            </div>
				    </li>
				--%></ul>
	    </div>
	    <div class="mui-content">
			<div style="padding: 10px 10px;">
				<div id="segmentedControl" class="mui-segmented-control">
					<a class="mui-control-item mui-active" href="#item1" onclick="switchtab(this)">
				基础耗材
			</a>
					<a class="mui-control-item" href="#item2" onclick="switchtab(this)">
				自定义耗材
			</a>
				</div>
			</div>
			...
			<!--end tab1 -->
			<!-- tab2-->
			<div id="item2"  style="display: none" class="mui-control-content mui-active">
				<form class="mui-input-group">
		            <div class="mui-input-row">
		                <label style="width: 30%;">明细&nbsp;:&nbsp;</label>
		                 <input type="text" name="tab2type" class="input"  placeholder="请输入耗材类型.." style="text-align: left;width: 70%;"/>
		            </div>
		             <div class="mui-input-row">
		                <label style="width: 30%;">规格&nbsp;:&nbsp;</label>
		                <input type="text" name="tab2specifications" class="input"  placeholder="请输入耗材规格.." style="text-align: left;width: 70%;"/>
		            </div>
		            <div class="mui-input-row">
		             	<label style="width: 30%;">价格&nbsp;:&nbsp;</label>
		                <input type="number" name="tab2iprice" class="input"  placeholder="请输入耗材费.." style="float: left;text-align: left;width: 34%;"/>
		                <button type="button" style="float: right;width: 25%;margin-top: 5px;" class="mui-btn-primary" onclick="selectvalueToaddgrid(2)"> 添加耗材</button>
		            </div>
		        </form>
			</div>
	    <!--end tab2 -->
        <%--<div class="title">&nbsp;</div> --%>
        <button type="button" onclick="subm()" style="padding: 10px;" class="mui-btn mui-btn-primary">保存</button>
...
<script type="text/javascript">
	...
	function selectvalueToaddgrid(tab){
		if(tab==1){
			var text=$(".map:last").find("option:selected").text();
			var value=$(".map:last").find("option:selected").val()+"";
			var price=$("[name=iprice]").val();
			var specifications=$("[name=specifications]").val();
		}else{
			var text=$("[name=tab2type]").val();
			var value="0";
			var price=$("[name=tab2iprice]").val();
			var specifications=$("[name=tab2specifications]").val();
		}
