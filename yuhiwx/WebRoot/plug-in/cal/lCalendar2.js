...
function editDatetime(e) {
    hlCalendar.listener = e.target;
    var a=hlCalendar.listener.attributes["show"];
    var nodeValue=hlCalendar.listener.attributes["show"].nodeValue;
    hlCalendar.listener.show=nodeValue;
    ...
    if (!hlCalendar.gearDate) {
        ...
        hlCalendar.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
            '<div class="date_btn_box">' +
            '<div class="date_btn" onclick="cancelDateEdit();">取消</div>' +
            '<div class="date_btn" id="edittime" onclick="finishDatetimeEdit();">确定</div>' +
            '</div>' +
            '<div class="date_roll_mask">' +
...
function finishDatetimeEdit(timeid) {
    hlCalendar.gearDate.style.display = "none";
    var date_yy = parseInt(hlCalendar.gearDate.querySelector(".date_yy").getAttribute("val"));
    var date_mm = parseInt(hlCalendar.gearDate.querySelector(".date_mm").getAttribute("val")) + minM - 1;
    date_mm = date_mm % 12 + 1;
    ...
    /*设置时间值指定控件*/
    document.getElementById(hlCalendar.listener.show).innerHTML =hlCalendar.listener.value;
}
//呼出时间插件
function editTime(e) {
    hlCalendar.listener = e.target;
