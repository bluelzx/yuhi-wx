...
import weixin.pay.beans.WechatOrder;
...
public class WechatOrderUtils {
	...
	public static synchronized Map<String, Object> CheckPayDetail(String orderid) {
		JSONObject result = new JSONObject();

        if (StringUtils.isBlank(orderid)) {
            Log.error("微信支付订单请求错误：请求参数不足", null);
        ...
        if ( StringUtils.isBlank(mchappid)
            ...
            Log.error("微信支付订单请求错误：系统配置信息缺失", null);
            result.put("status", "error");
     ...
     // 5、请求
        ...
        try {//注意，此处的httputil一定发送请求的时候一定要注意中文乱码问题，中文乱码问题会导致在客户端加密是正确的，可是微信端返回的是加密错误
           // response = HttpUtils.post(wx_order, xml);
            response =WeixinUtil.httpRequestByStr("https://api.mch.weixin.qq.com/pay/orderquery","POST", xml);
        } catch (Exception e) {
        ...
        Map<String, Object>  maps= Dom2Map(doc);
        return maps;
	}
    ...
    public static synchronized JSONObject createOrder(String detail, String desc, String openid, String ip, String goodSn, String orderSn, String amount, String type) {
       ...
       /* 公众号调起微信支付的时候，必须要有openID*/
        ...
        try {//注意，此处的httputil一定发送请求的时候一定要注意中文乱码问题，中文乱码问题会导致在客户端加密是正确的，可是微信端返回的是加密错误
           // response = HttpUtils.post(wx_order, xml);
            response =WeixinUtil.httpRequestByStr(wx_order,"POST", xml);
        } catch (Exception e) {
            Log.error("微信支付统一下单失败:http请求失败", e);
            result.put("status", "error");
            result.put("msg", "http请求失败");
